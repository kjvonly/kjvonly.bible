...
<div class="h-full w-full justify-start justify-items-start overflow-y-scroll">
	...
	<div class="mt-4 flex w-full flex-col justify-center p-6">
		<div class="flex w-full flex-row">
			<button
				onclick={() => {colorTheme.setLightTheme();
			...
			>
				...
				>
					<path
						class={chapterSettings?.colorTheme === 'light' ? 'fill-white' : 'fill-white'}
			...
			>
				...
				>
					<g id="g479" transform="translate(-83.478769,-13.695354)">
						<path
							class={chapterSettings?.colorTheme === 'dark' ? ' fill-white' : ''}
				...
				>
					...
					>
						<g id="g421" transform="translate(-59.92657,-22.570134)">
							<path
								class={chapterSettings?.fontSize === fs.fontSize ? 'fill-white' : 'fill-black dark:fill-gray-500'}
			...
			{#each fontFamilies as ff}
				<button
					...
					class="{chapterSettings?.fontFamily === ff.fontFamily
						? 'bg-black dark:bg-gray-500 text-white'
