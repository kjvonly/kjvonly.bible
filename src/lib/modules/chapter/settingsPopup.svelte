<script lang="ts">
	...
	import type { ChapterSettings } from '../../models/settings.model';
	...
	>
		<div class="flex w-full flex-row mt-4">
			<button
				...
				class=" {chapterSettings?.isDarkTheme === false
					? 'bg-primary-500 '
			...
			>
				...
				>
					<path
						class={chapterSettings?.isDarkTheme === true ? 'fill-neutral-700' : 'fill-neutral-100'}
			...
			<button
				onclick={() => {
					chapterSettings.isDarkTheme = true;
				}}
				class=" {chapterSettings?.isDarkTheme
					? 'bg-primary-500 '
					: ''} flex h-16 w-1/2 items-center justify-center border"
				aria-label="font-size button"
			>
				...
				>
					<g id="g479" transform="translate(-83.478769,-13.695354)">
						<path
							class={chapterSettings?.isDarkTheme === true
								? ' fill-neutral-100'
								: 'fill-neutral-700'}
