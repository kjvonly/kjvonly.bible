...
describe(
	...
	() => {
		...
		it(
			'recursively resolves a child descriptors Resource',
			async () => {
				...
				expect(
					nostrStrategy.resolve
				).toHaveBeenCalledWith(
					nestedDescriptor
				);

				expect(
					blossomStrategy.resolve
				).toHaveBeenCalledWith(
					planDescriptor
				);
			}
		...
		it(
			'rejects a recursive descriptors cycle',
			async () => {
				const root =
					createResourceRepresentation();
				...
				expect(
					...
					expect.objectContaining({
						message:
							`Recursive Resource descriptor cycle: ${root.publisher}/${root.resourceId}`
					})
				);
			}
		);
...
function createResolvedContent(
	descriptor:
		ResourceDescriptor,
