...
import { NostrClientError } from '$lib/infrastructure/nostr/client/nostr-client';
...
describe('RxNostrClient.getPublicKey', () => {
	it('delegates to the configured signer', async () => {
		const getPublicKey =
			vi.fn()
				.mockResolvedValue(
					'a'.repeat(64)
...
describe('RxNostrClient.getEvent', () => {
	it('returns null when no event is found', async () => {
		const rxNostr = createRxNostr([]);

		const client =
			createClient(rxNostr);
...
describe('RxNostrClient.getEvents', () => {
...
describe('RxNostrClient.setDefaultRelays', () => {
...
describe('RxNostrClient.dispose', () => {
...
describe('RxNostrClient.subscribe', () => {
	...
	it('throws when no readable relays are configured', () => {
		...
		).toThrow(NostrClientError);
	});
});

describe(
	'RxNostrClient.publishEvent',
	() => {
		it(
