import {
	...
	bookIDByshortBookNamesService,
	booksChaptersVerseCountByIDService,
	booksChaptersVerseCountByIDNumberService,
	shortBookNamesByIDService
} from '$lib/domains/bible/metadata';
import { describe, it } from 'vitest';

describe.skip('idByShortnames', () => {
	it('should output', () => {
		shortBookNamesByIDService.keys().forEach((k) => {
			console.log(
				`['${shortBookNamesByIDService.get(k).toLowerCase()}', ${k}],`
			);
		});
	});
});

describe.skip('createNTPlan', () => {
	it('should output', () => {
		createMonthNTPlan();
	});
});
		...
		return day
			...
			.reduce((p, c) => {
				return {
					...
					shortBooknamesChapters: (p.shortBooknamesChapters +=
						',' + c.shortBooknamesChapters)
				};
			});
	}).map((r, i) => {
		r.index = i;
...
function encodeBookChapters(
	...
	return {
		encodedReading: encodedReading,
		shortBooknamesChapters: reading,
...
function entireBook(shortBookName: string): Reading {
	let bookID = bookIDByshortBookNamesService.get(shortBookName);
	let bookChapters = booksChaptersVerseCountByIDService.get(`${bookID}`);
	let keys = bookChapters
		.keys()
		.toArray()
		.sort((a, b) => parseInt(a) - parseInt(b));
	let totalVerses = 0;
	let encodedReading = keys
		.map((ch) => {
	...
	return {
		encodedReading: encodedReading,
		shortBooknamesChapters: shortBookName,
		verseCount: totalVerses
	};
}

let NT_MEMORY_5_YEAR = [
	['1ti', '2ti', 'tit', 'eph', 'phi', 'heb', 'mar'],
	['1pe', '2pe', 'col', 'rom', '1th', '2th', '1co'],
	['2co', 'jam', 'gal', 'mat'],
	['phm', 'jude', '1jo', '2jo', '3jo', 'joh', 'rev'],
	['luk', 'act']
];
...
function getTotalVerses(books: string[]) {
	...
	books.forEach((b) => {
		...
		totalVerses += booksChaptersVerseCountByIDService
			.get(`${bookID}`)
			.entries()
			.map((c) => {
				return parseInt(c[1]);
...
function createBins(
	totalItems: number,
	verseCountPerDay: number,
	remainderPerDay: number
): number[] {
	const bins: number[] = [];
	let carry = 0;
...
function concatSynchronousReadings(bindedVerses: string[][]) {
	...
	bindedVerses.forEach((readings) => {
		...
		Object.keys(versesByBookChapter)
			.sort((a, b) => {
