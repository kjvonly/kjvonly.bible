...
export class PaneNavigationService {
	 ...
	 * Buffer persistence state refer to the same navigation-owned state.
	 */
	hydrate(
		navigationStates:
	...
	/**
	 * Clears this Pane's navigation stack so the underlying root Module can be
	 * rendered again.
	 */
	clear(): void {
		this.stack.hydrate([]);
