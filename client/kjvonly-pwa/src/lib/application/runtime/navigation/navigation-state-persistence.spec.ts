...
describe(
	...
	() => {
		...
		it(
			'leaves the legacy feature-specific navigation shape to its existing restore path',
			() => {
				expect(
					parseNavigationStates([
						{
...
describe(
	...
	() => {
		it(
			'appends the exact NavigationState object to Buffer state',
			() => {
				const bufferState:
					BufferState = {};
		...
		it(
			'clears the persisted navigation stack',
			() => {
				const persistWorkspace =
					vi.fn();
